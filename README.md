...
In your root project create new folder name `packages` and clone this repository to `packages` folder.

```bash
mkdir packages
cd packages
git clone https://github.com/minhlucvan/laravel-eme.git
```

Adding the package to your `composer,json`
...
Finally update composer packages
```bash
    composer update
```
